...
def train_model(model, train_dl, val_dl, opt, criterion, lr_start, lr_end=None, epochs=3, cycles=1, restarts=1, clip=50, print_epoch=True, is_classification=False, lr_cycle='cos', pprint=True):
    # try:
        train_loss = []
        val_loss = []
        ...
        #         if cycle>0: epochs = epochs*2
                if cycle: lr_start = lr_start*(0.99)
                ...
                for epoch in range(epochs):
                    ...
                    if lr_cycle == 'cos': opt.param_groups[0]['lr'] = lr_end + 0.5*(lr_start-lr_end)*(1+math.cos(epoch/epochs*np.pi))
                    if lr_cycle == 'sin': opt.param_groups[0]['lr'] = lr_start + (lr_end-lr_start)*(math.sin(epoch/epochs*np.pi))
                    running_loss = 0.0
                    model.train()
